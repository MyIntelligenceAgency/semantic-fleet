...
'm a
 writer";
    ...
    {
        //Arrange
        var oobaboogaSettings = new OobaboogaTextCompletionSettings(endpoint: this._endPointUri,
            blockingPort: BlockingPort, httpClient: this._httpClient,
        ...
        bool isChat = false)
    {
        ...
        if (isChat)
        {
            var oobaboogaSettings = new OobaboogaChatCompletionSettings(endpoint: new Uri("http://localhost/"),
        ...
        {
            var oobaboogaSettings = new OobaboogaTextCompletionSettings(endpoint: new Uri("http://localhost/"),
                streamingPort: StreamingPort,
                httpClient: this._httpClient,
                webSocketsCleanUpCancellationToken: cleanupToken.Token,
                webSocketFactory: webSocketFactory,
        ...
        if (isChat)
        {
            server = new OobaboogaWebSocketChatCompletionTestServer($"http://localhost:{StreamingPort}/", request => expectedResponse, logger: this._testOutputHelper)
            {
        ...
        {
            server = new OobaboogaWebSocketTextCompletionTestServer($"http://localhost:{StreamingPort}/", request => expectedResponse, logger: this._testOutputHelper)
            {
                RequestProcessingDelay = TimeSpan.FromMilliseconds(requestProcessingDuration),
                SegmentMessageDelay = TimeSpan.FromMilliseconds(segmentMessageDelay)
            };
